...
case "${model}" in
  afrcnn12)
    ...
    output_dir="experiments/seprune_afrcnn12_lrs2_mask_e07_seed2026"
    ;;
  sudormrf)
    checkpoint="experiments/original_sudormrf_lrs2_train/best.pt"
    output_dir="experiments/seprune_sudormrf_lrs2_mask_e07_seed2026"
    ;;
  *)
...
"${python_bin}" -u -m reproduction.train_mask_original \
  --model "${model}" \
